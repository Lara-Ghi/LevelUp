...
Run a group of tests in a single file:

```bash
php artisan test tests/Feature/RewardsTest.php
```
<!-- specify file path after test -->

Run a single test:

```bash
php artisan test --filter=test_only_non_archived_rewards_are_displayed 
```
<!-- put the name of the specific test after --filter=... -->
